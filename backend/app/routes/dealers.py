...
def invite_dealer(payload: DealerInviteCreate, db: Session = Depends(get_db)):
    ...
    if existing and existing.is_deleted:
        ...
        out = DealerOut.model_validate(existing)
        if getattr(settings, "DEBUG", False):
            return {**out.model_dump(), "debug_token": plain, "invite_link": link, "email_error": email_error}

        if email_error:
            raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Davet maili gönderilemedi: {email_error}")
    ...
    out = DealerOut.model_validate(user)
    if getattr(settings, "DEBUG", False):
        return {**out.model_dump(), "debug_token": plain, "invite_link": link, "email_error": email_error}
...
def resend_invite(dealer_id: UUID, db: Session = Depends(get_db)):
    ...
    if settings.DEBUG:
        return {"message": "Davet yeniden gönderildi", "debug_token": plain, "invite_link": link, "email_error": email_error}
...
def admin_setup_dealer(
    ...
    """
    Admin bayi için kullanıcı adı/şifreyi doğrudan ayarlar (opsiyonel mail gönderimi).
    """
    user = db.query(AppUser).filter(
        AppUser.id == dealer_id,
        AppUser.role == "dealer",
